...
class BlockLayered extends Module
{
	...
	public function indexAttribute($id_product = null)
	{
		...
		Db::getInstance()->execute('
			...
			SELECT pac.id_attribute, pa.id_product, ag.id_attribute_group, ags.`id_shop`
			...
			INNER JOIN '._DB_PREFIX_.'attribute_group_shop ags ON ag.id_attribute_group = ags.id_attribute_group
			'.(is_null($id_product) ? '' : 'AND pa.id_product = '.(int)$id_product).'
			GROUP BY a.id_attribute, pa.id_product , ags.`id_shop`'
		);
